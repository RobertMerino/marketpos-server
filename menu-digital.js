...
function selectType(type, el) {
    ...
    el.classList.add('active');
    document.getElementById('typeMesa').style.display = type === 'mesa' ? 'block' : 'none';
    document.getElementById('typeNombre').style.display = type !== 'mesa' ? 'block' : 'none';
    document.getElementById('typeTelefono').style.display = type !== 'mesa' ? 'block' : 'none';
    document.getElementById('typeDireccion').style.display = type === 'delivery' ? 'block' : 'none';
}

function renderProducts() {
    document.getElementById('productsGrid').innerHTML = getMenuData().filter(p => p.categoria === activeCategory).map(p => `
...
function updateCartFloat() {
    const c = cart.reduce((s, i) => s + i.cantidad, 0);
    const t = cart.reduce((s, i) => s + i.precio * i.cantidad, 0);
    const f = document.getElementById('cartFloat');
    if (c > 0) { f.style.display = 'flex'; document.getElementById('cartFloatCount').textContent = c; document.getElementById('cartFloatTotal').textContent = '$' + t.toFixed(2); }
    else { f.style.display = 'none'; }
}

function goToCart() { renderCart(); document.getElementById('modalCart').style.display = 'flex'; }
...
function renderCart() {
    ...
    document.getElementById('cartItems').innerHTML = cart.map((i, idx) => `
        <div class="cart-item-line">
            <div><strong>${i.emoji} ${i.nombre}</strong><br><small>$${i.precio.toFixed(2)}</small></div>
            <div style="display:flex;align-items:center;gap:8px;">
                <button class="btn-qty" onclick="cart[${idx}].cantidad--;if(cart[${idx}].cantidad<=0)cart.splice(${idx},1);renderCart();updateCartFloat();">−</button>
                <span>${i.cantidad}</span>
                <button class="btn-qty" onclick="cart[${idx}].cantidad++;renderCart();updateCartFloat();">+</button>
            </div>
        </div>
    `).join('');
}
...
async function confirmOrder() {
    ...
    const t = cart.reduce((s, i) => s + i.precio * i.cantidad, 0);
    const mesa = document.getElementById('typeMesa')?.value || '';
    const nombre = document.getElementById('typeNombre')?.value || 'Cliente';
    const telefono = document.getElementById('typeTelefono')?.value || '';
    const direccion = document.getElementById('typeDireccion')?.value || '';
    
    const pedido = {
        cliente: { nombre, mesa, telefono, direccion, tipoPedido: selectedType },
    ...
    document.getElementById('confirmDetails').innerHTML = `
        ...
        <p><strong>Tipo:</strong> ${selectedType === 'mesa' ? 'Mesa '+mesa : selectedType === 'llevar' ? 'Recoger' : 'Delivery'}</p>
        <p><strong>Total:</strong> $${t.toFixed(2)}</p>
    `;
